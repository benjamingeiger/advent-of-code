...
let findDuplicate banks =
    let rec go seen bs =
        ...
        if Set.contains nextCycle seen then (nextCycle, (Set.count seen)) else go (Set.add nextCycle seen) nextCycle

    go Set.empty banks

let findCycleLength banks =
    let (starting, _) = findDuplicate banks
    findDuplicate starting

let initBanks = (parse input)
printfn "%A" (findCycleLength initBanks)
