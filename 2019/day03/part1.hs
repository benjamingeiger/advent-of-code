...
parseWire s = ((head s, read (tail s)))

layWire :: (Int, Int, [Wire]) -> (Char, Int) -> (Int, Int, [Wire])
layWire (x,y,ws) (d,l)
    | d == 'U' || d == 'u' = (x, y + l, (VerticalWire x y (y + l)) : ws)
    | d == 'D' || d == 'd' = (x, y - l, (VerticalWire x (y - l) y) : ws)
    | d == 'L' || d == 'l' = (x - l, y, (HorizontalWire (x - l) x y) : ws)
    | d == 'R' || d == 'r' = (x + l, y, (HorizontalWire x (x + l) y) : ws)
    | otherwise = (x, y, ws)

splitWires :: String -> [Wire]
splitWires s = let (_, _, result) = foldl layWire (0, 0, []) $ map parseWire $ wordsWhen (==',') s in result

wires1 = splitWires puzzleInput1
wires2 = splitWires puzzleInput2

intersectWires :: Wire -> Wire -> Maybe (Int, Int)
intersectWires (HorizontalWire _ _ _) (HorizontalWire _ _ _) = Nothing
intersectWires (VerticalWire _ _ _) (VerticalWire _ _ _) = Nothing
intersectWires (HorizontalWire x1l x1h y1) (VerticalWire x2 y2l y2h) = 
    if x1l <= x2 && x2 <= x1h && y2l <= y1 && y1 <= y2h then Just (x2, y1) else Nothing
intersectWires (VerticalWire x2 y2l y2h) (HorizontalWire x1l x1h y1) = 
    if x1l <= x2 && x2 <= x1h && y2l <= y1 && y1 <= y2h then Just (x2, y1) else Nothing

intersections :: [Wire] -> [Wire] -> [(Int, Int)]
intersections ws1 ws2 = catMaybes [intersectWires x y | x <- ws1, y <- ws2]

result = sort $ map (\(x, y) -> (abs x) + (abs y)) $ intersections wires1 wires2
