...
let (_, _, naps) = recordNaps records

let (biggestNapper, _) =
    naps
    |> Map.toSeq
    |> Seq.map (fun (i, ns) -> (i, Seq.map (fun (x, y) -> y - x) ns))
    |> Seq.map (fun (i, ns) -> (i, Seq.sum ns))
    |> Seq.maxBy snd

printfn "%A" biggestNapper

let bigNaps = Map.find biggestNapper naps
...
(*printfn "%A" (biggestMinute bigNaps)*)
printfn "%A" ((biggestMinute bigNaps) * biggestNapper)
