...
let tick (map : Cell [,]) carts =
    ...
    let rec go uncrashed crashed toProcess =
        match toProcess with
        | [] -> (List.rev uncrashed, List.rev crashed)
        | cur :: rest ->
            ...
            then
                go (updated :: uncrashed) crashed rest
            else
                go (List.except crashed' uncrashed) (List.append crashed' crashed) (List.except crashed' rest)

    carts
    |> List.sortBy (function | Cart (_, (x, y), _) -> (y, x))
    |> go [] []

let rec simulate map carts =
    printfn "%A" carts

    let (uncrashed, _) = tick map carts
    match uncrashed with
    | [] -> failwith "ran out of carts"
    | [c] -> c
    | cs -> simulate map cs

let remainingCart = simulate map carts
printfn "%A" remainingCart
