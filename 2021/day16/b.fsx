...
let rec execute = function
    ...
    | Operator (_, opcode, ps) ->
        let subPackets = List.map execute ps

        match opcode with
        | 0 -> List.sum subPackets
        | 1 -> List.reduce ( * ) subPackets
        | 2 -> List.min subPackets
        | 3 -> List.max subPackets
        | 5 -> if List.item 0 subPackets > List.item 1 subPackets then 1I else 0I
        | 6 -> if List.item 0 subPackets < List.item 1 subPackets then 1I else 0I
        | 7 -> if List.item 0 subPackets = List.item 1 subPackets then 1I else 0I
...
let result =
    ...
    let returnValue = execute packet
