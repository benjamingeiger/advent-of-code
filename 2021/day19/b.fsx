...
let findMatchingScanners existingBeaconSets scanSet =
    ...
    |> List.tryPick (fun s ->
        scanSet
        |> List.tryPick (fun ss -> attemptScannerMatch s ss))

let attachScanners init scanSets =
    let rec go (existingScanners, unmatched) scanSets =
        printfn "Iterating: %A existing scanners, %A unmatched scan sets" (List.length existingScanners) (List.length scanSets)
        ...
        |> List.fold (fun (existingScanners', unmatched') scanSet ->
            ...
            | Some scans ->
                printfn "Found a scanner at %A" (fst scans)
                (scans :: existingScanners', unmatched')
            | None -> (existingScanners', scanSet :: unmatched')) (existingScanners, [])
        |> function
            | (scanners, []) -> scanners
            | (scanners, unmatched'') ->
...
let result =
    ...
    let furthestScanners =
        List.allPairs allScanners allScanners
        |> List.map (fun (x, y) -> manhattan x y)
        |> List.max

    let returnValue = furthestScanners
