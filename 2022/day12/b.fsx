...
let bfs map start goal =
    ...
    let rec step searched = function
        | [] -> None
        | ((r, c), n, prev) :: queue ->
            if (r, c) = goal then Some (n, Map.add (r, c) prev searched)
...
let run input =
    let starts = input |> Map.filter (fun _ v -> v = 'S' || v = 'a') |> Map.keys |> Seq.toList
    let goal = input |> Map.findKey (fun _ v -> v = 'E')

    starts
        |> List.choose (fun start -> bfs input start goal)
        |> List.map fst
        |> List.min
