...
let bfs squares startingBlizzards start goal =
    ...
    let rec step visited = function
        ...
        | (i, map, pos, goals) :: queue ->
            if (Set.contains (i, pos, goals) visited) then step visited queue else

            if goals = [pos] then i else

            let goals' = if pos = List.head goals then List.tail goals else goals
            ...
            let neighbors' =
                ...
                |> List.map (fun pos' -> i + 1, newMap, pos', goals')

            if goals = goals' then
                step (Set.add (i, pos, goals') visited) (queue @ neighbors')
            else
                step (Set.singleton (i, pos, goals')) neighbors'

    step Set.empty [(0, startingBlizzards, start, [goal; start; goal])]
