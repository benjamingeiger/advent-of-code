...
let rec doStep count order state = 
    ...
    if proposedMotions = [] || count <= 0 then state else
...
let countEmpty state =
    let elves = state |> Set.toList
    let minRow = elves |> List.minBy fst |> fst
    let maxRow = elves |> List.maxBy fst |> fst
    let minCol = elves |> List.minBy snd |> snd
    let maxCol = elves |> List.maxBy snd |> snd

    let total = (maxRow - minRow + 1) * (maxCol - minCol + 1)
    total - (Set.count state)

let run input =
    input
    |> doStep 10 considerationOrder
    |> countEmpty
